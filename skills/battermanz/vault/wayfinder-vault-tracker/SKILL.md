...
- **Frontier**: a ticket is on the frontier when open, unblocked (every `Blocked by:` ticket resolved), and unclaimed; first by number wins. List `wayfinder/<effort>/issues/` to scan. During or just after a write burst, per-note reads are authoritative and `get_tree` is not (Hatchdoor #226).
...
Ticket resolutions are **waypoints**: while the effort is live, a decision's only home is its ticket. Only when the map fully resolves, with nothing left to decide, is the decision reached. Then:
...
Every write: `wayfinder(<effort>): <what happened>`. That is the effort's audit trail in the vault's git history.
