...
- **Vault work** → **`/hatchdoor`** for any BatterNotes read/write, **`/clip-web-article`** to archive an article with its images, **`/deep-research`** for a multi-agent research dossier landed in the vault, **`/sci-hub`** when a source stayed unread behind a paywall or bot wall (the "sources to obtain" lists), **`/wayfinder-vault-tracker`** when a `/wayfinder` effort lives in the vault instead of a repo tracker. `effort-store` and `effort-store-vault` are plumbing other skills load, not entry points.
