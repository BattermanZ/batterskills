...
- **Read back every save** with `get_recipe`: ingredient count, step count, and the fields you meant to leave alone. Move on only when they match.
...
- **Steps are actions**, one per step, in the source's wording. Filler such as "Enjoy!" is dropped; a step that is really an ingredient moves to the ingredients.
...
A recipe that is an empty bookmark, a caption dumped into one step, or a method filed as ingredients is rebuilt from its own source, never from memory. How to reach each kind of source (web pages, YouTube descriptions, captions and on-screen cards, Instagram accents) is in [references/recovering-recipes.md](references/recovering-recipes.md).
